...
public class QuestADBWrapper {
    ...
    public static final String questAddress = "10.105.89.21:5555";
    ...
    public static QuestNavADBCommandResult sendQuestNavADBCommand(String... command) {
        ...
        try {
            command = add2BeginningOfArray(command, "bash", "-c", adbExecPath);
            currentADBCommand = Runtime.getRuntime().exec(command);
    ...
    public static void waitForCurrentADBCommand() {
        try {
            currentADBCommand.waitFor(250, TimeUnit.MILLISECONDS);
